...
export default function Quiz() {
  ...
  const saveScoreAsPng = () => {
    if (score && resultRef.current) {
      html2canvas(resultRef.current, {
        backgroundColor: '#000', 
        scale: 2, 
      }).then((canvas) => {
        const link = document.createElement('a');
        link.download = 'retard-score.png';
        link.href = canvas.toDataURL('image/png');
        link.click();
      }).catch((err) => {
        console.error('Failed to save PNG:', err);
        alert('Oops, PNG save failed, bruh! Try again!');
      });
    }
  };
      ...
      {score === null ? (
        ...
        <div className="result" ref={resultRef}>
          ...
          <div className="result-buttons">
            <button onClick={saveScoreAsPng} className="save-btn">
              SAVE AS PNG, BRO
            </button>
